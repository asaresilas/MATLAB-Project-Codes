...
const normalizeLivePayload = (payload, machineId, previousSensors, previousOperatingPoint) => {
  if (payload.type === 'dashboard_update') {
    return {
      ...
      // Every simultaneously-active fault, each with its own name and explanation.
      faults:          payload.faults || payload.machine?.faults || [],
      fault_count:     payload.fault_count ?? (payload.faults?.length ?? 0),
      machine: {
...
export function useDashboardController() {
  ...
  const applySample = (sample, sourceMode) => {
    ...
    const nextMachine = {
      ...
      faults:        sample.faults || sample.machine?.faults || [],
      faultCount:    sample.fault_count ?? (sample.faults?.length ?? 0),
